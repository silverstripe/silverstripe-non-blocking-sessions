...
// We can't use PHPUnit 10+ because of a dependency conflict with `sebastian/diff` which is
// a dependency of `silverstripe/framework`.
// After headers have been sent, we can't call `session_set_save_handler()` anymore without
// PHP complaining about it, so we
// need to avoid calling that method after headers have been sent.
if (headers_sent($filename, $line)) {
    // since we're handling this scenario here we should log it just in case something weird
    // happens during non-test execution
